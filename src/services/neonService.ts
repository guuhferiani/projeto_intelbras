...
export async function saveDataToNeon(payload: SaveDataPayload): Promise<{ success: boolean; message: string }> {
  // 1. Try via Vercel Backend API (/api/data POST)
  try {
    const res = await fetch('/api/data', {
      method: 'POST',
    ...
    // Fall back to direct Neon SQL if running locally
  }

  // 2. Direct client-side Neon SQL (Local dev fallback)
  ...
  return { success: true, message: 'Dados carregados na sessão.' };
}
