...
export const RelatorioFinalTab: React.FC<RelatorioFinalTabProps> = ({ records, darkMode }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3.5">
        ...
        <div className="glass-card glass-card-hover p-4 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Avaliados</span>
        ...
        <div className="glass-card glass-card-hover p-4 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Média de Notas</span>
        ...
        <div className="glass-card glass-card-hover p-4 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Frequência Média</span>
        ...
        <div className="glass-card glass-card-hover p-4 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Aprovação</span>
        ...
        <div className="glass-card glass-card-hover p-4 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Horas Capacitadas</span>
        ...
        <div className="glass-card glass-card-hover p-4 relative overflow-hidden group">
          ...
          <div className="flex items-center justify-between mb-1.5">
            <span className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider">Corpo Docente</span>
            <div className="w-7 h-7 rounded-lg bg-amber-500/15 text-amber-400 flex items-center justify-center">
              <UserCheck className="w-4 h-4" />
            </div>
          </div>
          <div className="text-2xl font-black text-amber-300 tracking-tight">
            {kpis.totalDocentes} <span className="text-xs font-normal text-gray-400">professores</span>
          </div>
          <div className="text-[11px] text-gray-400 mt-1">
      ...
      <div className="glass-card p-4 border border-white/10 shadow-lg">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 text-xs">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => handleFilterChange('faixaNota', e.target.value)}
              className="w-full bg-gray-900/70 border border-white/10 rounded-lg px-2.5 py-1.5 text-xs text-white focus:outline-none focus:border-[#00A335] transition-colors cursor-pointer"
            >
              <option value="all">Todas as Notas</option>
              <option value="90-100">90 a 100 (Excelente)</option>
              <option value="80-89">80 a 89 (Bom)</option>
              <option value="70-79">70 a 79 (Regular)</option>
        ...
        <div className="glass-card p-5">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-gray-400">Comparativo de aproveitamento acadêmico e presença</p>
            </div>
          </div>
          <div className="h-72">
        ...
        <div className="glass-card p-5">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-gray-400">Média de notas dos alunos por instrutor responsável</p>
            </div>
          </div>
          <div className="h-72">
        ...
        <div className="glass-card p-5">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-gray-400">Percentual de alunos por patamar de pontuação final</p>
            </div>
          </div>
          <div className="h-72">
        ...
        <div className="glass-card p-5">
          <div className="flex items-center justify-between mb-3">
            <div>
              ...
              <p className="text-xs text-gray-400">Impacto da assiduidade no rendimento dos estudantes</p>
            </div>
          </div>
          <div className="h-72">
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pb-4 mb-4 border-b border-white/10">
          <div>
            ...
            <p className="text-xs text-gray-400">
              Exibindo <strong>{filteredRecords.length}</strong> alunos com formação concluída
            </p>
          </div>
          ...
          <table className="w-full text-left text-xs border-collapse">
            <thead>
              <tr className="border-b border-white/10 text-gray-400 font-semibold bg-white/5">
                ...
                <th onClick={() => handleSort('docente')} className="p-2.5 cursor-pointer hover:text-white">Docente</th>
                <th onClick={() => handleSort('notaFinal')} className="p-2.5 cursor-pointer hover:text-white text-right">Nota Final</th>
                <th onClick={() => handleSort('frequencia')} className="p-2.5 cursor-pointer hover:text-white">Frequência</th>
                <th onClick={() => handleSort('resultadoFinal')} className="p-2.5 cursor-pointer hover:text-white">Resultado</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-white/5">
              {paginatedRecords.map((r) => {
                const nota = r.notaFinal;
                let notaBadge = 'bg-emerald-500/20 text-emerald-300 border-emerald-500/40';
                if (nota < 80) notaBadge = 'bg-amber-500/20 text-amber-300 border-amber-500/40';
                if (nota < 70) notaBadge = 'bg-rose-500/20 text-rose-300 border-rose-500/40';

                return (
                  <tr key={r.id} className="hover:bg-white/5 transition-colors">
                    <td className="p-2.5 font-mono text-[#00A335] font-bold">{r.matricula}</td>
                    <td className="p-2.5 text-gray-400 font-mono text-[11px]">{r.cpf}</td>
                    <td className="p-2.5 font-semibold text-white">
                      {r.nome}
                    </td>
                    <td className="p-2.5 text-gray-200 max-w-xs truncate" title={r.curso}>
                      {r.curso}
                    </td>
                    <td className="p-2.5 font-mono text-cyan-300 font-medium">{r.turma}</td>
                    <td className="p-2.5 text-gray-300">{r.cargaHoraria}h</td>
                    <td className="p-2.5 text-gray-300 text-[11px] font-medium">{r.docente}</td>
                    ...
                    <td className="p-2.5">
                      <div className="flex items-center gap-2">
                        <div className="w-16 bg-gray-700 h-2 rounded-full overflow-hidden">
                          <div 
                            className="bg-[#00A335] h-full rounded-full"
                            style={{ width: `${r.frequencia}%` }}
                          />
                        </div>
                        <span className="font-mono text-[11px] text-gray-300">{r.frequencia}%</span>
                      </div>
                    </td>
                    <td className="p-2.5">
